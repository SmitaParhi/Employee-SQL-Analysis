...
SELECT concat(FIRST_NAME, " ", LAST_NAME) AS "Employee name", (1.15*SALARY) AS "Salary" 
FROM employees;


--  Q4 Write a SQL query to list the employee's name and job name as a format of "Employee & Job".
SELECT concat(employees.FIRST_NAME, " ", employees.LAST_NAME, "  &  ", jobs.JOB_TITLE) AS "Employee name & Job" 
...
SELECT EMPLOYEE_ID, concat(FIRST_NAME, " ", LAST_NAME) AS "Employee name", SALARY, DATE_FORMAT(HIRE_DATE, ' %M %D, %Y') AS hire_date 
from employees;
...
select distinct employees.DEPARTMENT_ID,jobs.JOB_TITLE 
from employees JOIN jobs on (employees.JOB_ID = jobs.JOB_ID);
